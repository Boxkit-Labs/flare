  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    const challenge = {
      ...
      resource: {
        ...
        description: "Premium product data access",
 ...
 */
app.post('/api/products', x402Protector("60000"), (req: Request, res: Response) => {
  const { product_name, target_price } = req.body;
  ...
  if (!result) {
    res.status(404).json({ error: `Product not found: ${product_name}` });
    return;
  }

  // Optional: Compare with target_price
  const isBelowTarget = target_price ? result.current_price <= target_price : null;
