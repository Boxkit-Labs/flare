  ...
  void _refreshData({bool force = false}) {
    ...
    if (authState is AuthAuthenticated) {
      ...
      context.read<WalletBloc>().add(
        LoadAllWalletData(userId, isRefresh: isRefresh),
      );
      context.read<WatchersBloc>().add(
        LoadWatchers(userId, isRefresh: isRefresh),
      );
      context.read<FindingsBloc>().add(
        LoadFindings(userId, isRefresh: isRefresh),
      );
      context.read<BriefingBloc>().add(
        LoadTodayBriefing(userId, isRefresh: isRefresh),
      );
