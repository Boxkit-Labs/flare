...
async function runTest() {
    ...
    try {
        ...
        console.log('[Phase 2] Assembled transaction type:', (assembledTx as any).constructor.name);
        ...
        try {
            ...
            console.log('[Phase 2] toXDR() failed, trying toEnvelope().toXDR():', e.message);
    ...
    return json.result;
        ...
        if (replayResponse.status === 402) {
            ...
            if (err.error === 'tx_already_used') {
                ...
                console.log('❌ Replay check failed with unexpected error:', err.error);
            }
        } else {
