...
export class MPPStreamService {
        ...
        try {
            ...
            if (message.type === 'payment_proof' && message.proof) {
                ...
                if (isValid) {
                    ...
                    client.ws.send(JSON.stringify({ type: "payment_ack", message: "Proof accepted" }));
                } else {
                    client.ws.send(JSON.stringify({ type: "error", message: "Invalid payment proof" }));
                    client.ws.close(1008);
                    this.handleDisconnect(clientId);
                }
            }
    ...
    private handleDisconnect(clientId: string) {
        ...
        if (client) {
            ...
            if (client.paymentTimeoutId) clearTimeout(client.paymentTimeoutId);
            console.log(`[MPP Stream] Client disconnected: ${clientId}. Total Cost: ${client.cumulativeCost}`);
